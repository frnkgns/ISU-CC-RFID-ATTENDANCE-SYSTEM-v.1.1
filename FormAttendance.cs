...
namespace RFID_Attendance_System
{
    ...
    class formAttendanceCR : cardReader
    {
        ...
        static string dbname = @"RFID DB.db";
        static string dbpath = Path.Combine(Application.StartupPath, dbname);
        ...
        public override void ChangeStatus()
        {
            ...
            {
                conn.Open();
                //wag nyo remove to kahit na double sya sa baba kasi minsan hindi nakukuha yung title event, this method works, pero try nyo if kaya
        ...
        public void AddAttendaceToDatabase()
        {
            ...
            string Check = $"select StudentID from attendance where EventId = @Evntid and StudentID = '{stdntid}'";
            ...
            if (stdntid.Length > 0 && name.Length > 0 && course.Length > 0 && timein.Length > 0)
            {
                ...
                {
                    cmd.Parameters.AddWithValue("@Evntid", eventid);
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
                    {
