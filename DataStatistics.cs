...
namespace RFID_Attendance_System
{
    ...
    public partial class DataStatistics : Form
    {
        ...
        public static string dbpath = dbpath = Path.Combine(dbname);
        ...
        public void LoadData(string event_date)
        {
            string[] Sections = {
            ...
            using (SQLiteConnection conn = new SQLiteConnection($"Data Source= {dbpath}; Version= 3;"))
            {
                ...
                for (int i = 0; i < Sections.Length; i++)
                {
                    ...
                    string GetData = $"select * from attendance where EventID = @Event_ID and Course = '{Sections[i]}'";
                    using (SQLiteCommand cmd = new SQLiteCommand(GetData, conn))
                    {
                        cmd.Parameters.AddWithValue("@Event_ID", event_date);
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                ...
                for (int i = 1; i < 5; i++)
                {
                    string StudentPerYear = $"select * from attendance where EventID = @Event_ID and Course like '%{i}%'";
                    using (SQLiteCommand cmd1 = new SQLiteCommand(StudentPerYear, conn))
                    {
                        cmd1.Parameters.AddWithValue("@Event_ID", event_date);
                ...
                for (int i = 0; i < 2; i++)
                {
                    string GetLate = $"select * from attendance where EventID = @Event_ID and late = '{late[i]}'";
